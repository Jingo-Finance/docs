...
`Observation`s may be retrieved via the [`observations`](../../contracts/v3/reference/core/interfaces/pool/IJingoV3PoolState#observations) method on v3 pools. However, this is _not_ the recommended way to consume oracle data. Instead, prefer [`observe`](../../contracts/v3/reference/core/JingoV3Pool#observe):
...
See [OracleLibrary](https://github.com/Jingo-Finance/v3-periphery/blob/main/contracts/libraries/OracleLibrary.sol) for an example of how to use the tick accumulator.
