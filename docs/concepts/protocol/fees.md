...
Swap fees are not automatically reinvested as they were in previous versions of Jingo. Instead, they are collected separately from the pool and must be manually redeemed when the owner wishes to collect their fees.

## Pool Fees Tiers

Jingo v3 introduces multiple pools for each token pair, each with a different swapping fee. Liquidity providers may initially create pools at three fee levels: 0.05%, 0.30%, and 1%. More fee levels may be added by PSYS governance.
...
Jingo v3 has a protocol fee that can be turned on by governance. Compared to v1, governance has more flexibility in choosing the fraction of swap fees that go to the protocol. 
