...
This guide walks you through the steps to embed the swap widget in your website in 2 minutes — whether your site is already a decentralized application (dApp) or not. With the swap widget, your users can trade ERC-20 tokens on the Jingo Protocol without leaving your site!
...
<iframe
  ...
  height="376"
  scrolling="no"
  src="https://widgets-jingo.vercel.app/_renderer.html?_fixtureId=%7B%22path%22%3A%22src%2Fcosmos%2FSwap.fixture.tsx%22%2C%22name%22%3Anull%7D"
></iframe>

Example use cases include:

- Building a custom frontend for the Jingo Protocol
...
```bash npm2yarn
npm install --save @jingofi/widgets react-redux
```
...
```jsx
import { SwapWidget } from '@jingofi/widgets'
import '@jingofi/widgets/fonts.css'
...
That’s it! You should now see a fully functional swap widget on your site. The widget is self-contained and gracefully handles all interactions with the Jingo Protocol. It leverages the [Auto Router](/sdk/v3/guides/routing) to compute the best price across all Jingo v1 and v3 pools.
...
```jsx
import { SwapWidget } from '@jingofi/widgets/dist/index.js'
import '@jingofi/widgets/dist/fonts.css'
```
...
```tsx
import { Theme, SwapWidget } from '@jingofi/widgets'
...
The swap widget fonts must be imported explicitly with `import '@jingofi/widgets/fonts.css'`. However, if you’re overriding the swap widget fonts with your own, or with a default font such as Helvetica in the example above, you can remove this `import` statement.
:::
...
```tsx
import { darkTheme, lightTheme, Theme, SwapWidget } from '@jingofi/widgets'
import '@jingofi/widgets/fonts.css'
...
By default, the widget will use the [Uniswap Labs default token list](https://tokenlists.org/token-list?url=https://gateway.ipfs.io/ipns/tokens.jingo.fi) in the selector.
...
const CMC_TOKEN_LIST = 'https://api.coinmarketcap.com/data-api/v3/jingofi/all.json'
...
const UNISWAP_TOKEN_LIST = 'https://gateway.ipfs.io/ipns/tokens.jingo.fi'
