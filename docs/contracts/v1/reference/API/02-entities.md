...
Entities define the schema of the subgraph, and represent the data that can be queried. Within each entity are sets of fields that store useful information related to the entity. Below is a list of the available entities within the Jingo Subgraph, and descriptions for the available fields.

To see an interactive sandbox of all entities see the [Graph Explorer](https://thegraph.com/explorer/subgraph/jingofi/v1).
...
The Jingo Factory entity is responsible for storing aggregate information across all Jingo pairs. It can be used to view stats about total liquidity, volume, amount of pairs and more. There is only one JingoFactory entity in the subgraph.

| Field Name        | Value Type | Description                                                     |
| ----------------- | ---------- | --------------------------------------------------------------- |
| id                | ID         | factory address                                                 |
| pairCount         | Int        | amount of pairs created by the Jingo factory                  |
...
| factory              | JingoFactory      | reference to Jingo factory entity                                                                                 |
...
A user entity is created for any address that provides liquidity to a pool on Jingo. This entity
...
Transaction entities are created for each Ethereum transaction that contains an interaction within Jingo contracts. This subgraph tracks Mint, Burn, and Swap events on the Jingo core contracts. Each transaction contains 3 arrays, and at least one of these arrays has a length of 1.
...
Mint entities are created for every emitted Mint event on the Jingo core contracts. The Mint entity stores key data about the event like token amounts, who sent the transaction, who received the liquidity, and more. This entity can be used to track liquidity provisions on pairs.
...
Burn entities are created for every emitted Burn event on the Jingo core contracts. The Burn entity stores key data about the event like token amounts, who burned LP tokens, who received tokens, and more. This entity can be used to track liquidity removals on pairs.
...
| mostLiquidTokens  | [TokenDayData!] | tokens with most liquidity in Jingo                                            |
