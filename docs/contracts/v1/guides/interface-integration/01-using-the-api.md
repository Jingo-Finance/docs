...
In this guide we will create a web interface that consumes and displays data from the Jingo Subgraph. The goal is to provide a quick overview of a setup that you can extend to create your own UIs and analytics around Jingo data.
...
```javascript
yarn create react-app jingo-demo
cd jingo-demo
...
We need to set up some middleware in order to make requests to the Jingo subgraph and receive data. To do this we'll use Apollo and create a graphql client to handle this.

1. Add the imports shown below and instantiate a new client instance. Notice how we use the link to the Jingo subgraph here.
...
export const client = new ApolloClient({
  link: new HttpLink({
    uri: 'https://api.thegraph.com/subgraphs/name/jingofi/v1',
  }),
...
Next we'll construct our query and fetch data. For this example we will fetch some data about the Dai token on Jingo V1. We'll get the current price, and total liquidity across all pairs. We'll be using the Dai address as an id in this query. We'll also fetch the USD price of ETH to help create USD conversion for Dai data.
...
Now we're ready to use these queries to fetch data from the Jingo V1 subgraph. To do this we can use the `useQuery` hook which uses our client instance to fetch data, and gives us live info about the status of the request. To do this add the following to your `App.js` file:
...
This should render a very basic page with these two stats about the Dai token within Jingo. This is a very basic example of what you can do with the Jingo subgraph and we encourage you to build out more complex and interesting tools!

You can visit our [analytics site](https://jingo.fi.info/) to see a more advanced analytics page, or visit [the github](https://github.com/Jingo/jingo-info) for more detailed examples of using the Jingo subgraph to create UIs.
