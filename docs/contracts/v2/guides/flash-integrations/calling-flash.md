...
The `FlashCallbackData` struct will contain the data we want to send to the callback. This includes `poolKey`, which expresses the sorted tokens with the matched fee tier, returned by the [**PoolAddress**](https://github.com/Jingo-Finance/v3-periphery/blob/main/contracts/libraries/PoolAddress.sol) library.
...
Next we will declare `pool` as type [**IJingoV3Pool**], which allows us to call `flash` on our desired pool contract.

```solidity
        IJingoV3Pool pool = IJingoV3Pool(PoolAddress.computeAddress(factory, poolKey));
```
